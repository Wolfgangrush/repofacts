...
Public dataclasses:
    ...
    SecurityFacts / SecurityReport   — deep-check inputs/outputs
    QualityFacts  / QualityReport     — deep-check inputs/outputs
"""
...
# existing fields. SecurityFacts and QualityFacts are populated by
# ``github.py`` (the only network module) and consumed by the pure
# ``security.py`` and ``quality.py`` assessors.
#
...
@dataclass
class SecurityFacts:
    """Network-layer output for security checks.

    Every field is ``None`` / empty until :func:`repofacts.github.fetch_security_facts`
    populates it. A field that is ``None`` after the fetcher ran means the
    data was not obtainable (404, permission, network); the assess layer
    treats that as ``unchecked``, never as ``pass``.
...
@dataclass
class QualityFacts:
    """Network-layer output for quality checks.

    Mirrors :class:`SecurityFacts` semantics: ``None`` and empty mean
    "the fetcher could not get this", which the assessor maps to
    ``unchecked``.
    """
    owner: str
    repo: str
    ...
    #: Repo tree (top 2 levels). Used by TestsPresent.
    tree_entries: list[dict] = field(default_factory=list)
    ...
    #: ``path -> content`` for workflow YAML. Used by CIConfigured and
    #: could be used for deeper checks in the future.
    workflow_files: dict[str, str] = field(default_factory=dict)

    #: Recent releases (full objects). Used by ReleaseCadence.
    releases: list[dict] = field(default_factory=list)

    #: Tag names from ``/releases`` or ``/tags``. Used by SemVerAdherence.
    tags: list[str] = field(default_factory=list)

    #: ``/contributors`` entries. Used by ContributorConcentration as a
    #: fallback if recent commits are unavailable.
    contributors: list[dict] = field(default_factory=list)
    ...
    #: Manifest files: ``path -> content``. Used by DependencyWeight.
    manifests: dict[str, str] = field(default_factory=dict)
